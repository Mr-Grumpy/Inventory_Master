import os, random, asyncio, time, datetime, openpyxl, pprint, sys, queue, dotenv, json
...
def load():
    ...
    days_ = 0
    yesterday = today - datetime.timedelta(days=days_)
    while os.path.isfile('invMaster - ' + str(yesterday) + '.xlsx') is False:
        yesterday = today - datetime.timedelta(days=days_)
        print('Nope')
        days_ = days_ + 1
        print(days_)
        print(yesterday)
    while os.path.isfile('invMaster - ' + str(yesterday) + '.xlsx') is True:
        print('Nice')
        global workbook
        workbook = openpyxl.load_workbook('invMaster - ' + str(yesterday) + '.xlsx')
        start_up()
...
def start_up():
    try:
        global workbook
        workbook.save(filename = 'invMaster - ' + str(today) + '.xlsx')
        workbook.close()
        workbook = openpyxl.load_workbook('invMaster - ' + str(today) + '.xlsx')
